...
router.get('/filtration/:category', function (req, res, next) {
    ...
    let filterQuery = _.cloneDeep(req.query);

    if (req.params.category) {
        myFilter.setCategories(req.params.category);
    }

    if (filterQuery.sort || filterQuery.sortBy) {
    ...
    _(filterQuery).forEach(function (value,key) {
        myFilter.setProperties(key, value);
    })
    ...
    Filters.find({'_id': req.params.category})
        .then(result => {
            ...
            let newResult = _.cloneDeep(result[0]);
            ...
            function forProperties(index) {
                ...
                function forValue(valueIndex) {
                    ...
                    function getCount(index, valueIndex) {
                        ...
                        let newQuery = _.cloneDeep(myFilter.creatQuery());
                        let query = [{
                        ...
                        let currentQuery = _.cloneDeep(myFilter.creatQuery());
                        currentQuery['$and'][2] = newQuery['$and'][2];
                        _(currentQuery['$and'][1]['$and']).forEach(function (property) {
...
router.get('/filter/', function (req, res, next) {
    ...
    if (filterQuery.sort || filterQuery.sortBy) {
        myFilter.setSort(filterQuery.sort,filterQuery.sortBy);
        delete filterQuery.sort
        delete filterQuery.sortBy
    }
